...
function findLTPatterns(sets) {
    let patterns = [];
    let hSets = sets.filter(s => s.orientation === 'horizontal');
    let vSets = sets.filter(s => s.orientation === 'vertical');
    for (let hSet of hSets) {
        for (let vSet of vSets) {
            let shared = null;
            for (let hGem of hSet.gems) {
                shared = vSet.gems.find(v => v.r === hGem.r && v.c === hGem.c);
                if (shared) break;
            }
            if (!shared) continue;
            let hCols = hSet.gems.map(g => g.c);
            let isHEnd = shared.c === Math.min(...hCols) || shared.c === Math.max(...hCols);
            let vRows = vSet.gems.map(g => g.r);
            let isVEnd = shared.r === Math.min(...vRows) || shared.r === Math.max(...vRows);
            patterns.push({ intersection: shared, shapeType: isHEnd && isVEnd ? 'L' : 'T' });
        }
    }
    return patterns;
}
...
function findSquares() {
    ...
    for (let r = 0; r < GRID_ROWS - 1; r++) {
        for (let c = 0; c < GRID_COLS - 1; c++) {
            ...
            if (type === GEM_ROCK) continue;
            if (state.grid[r][c + 1].type === type &&
                state.grid[r + 1][c].type === type &&
                state.grid[r + 1][c + 1].type === type) {
                squares.push([
                    { r, c },
                    { r, c: c + 1 },
                    { r: r + 1, c },
...
async function handleMatches() {
    ...
    // L/T Shape detection
    let ltPatterns = findLTPatterns(sets);
    ltPatterns.forEach(lt => {
        showMessage(lt.shapeType === 'L' ? "L-SHAPE!" : "T-SHAPE!");
        updateScore(1000);
        for (let c = 0; c < GRID_COLS; c++) toRemove.push({ r: lt.intersection.r, c });
        for (let r = 0; r < GRID_ROWS; r++) toRemove.push({ r, c: lt.intersection.c });
    });
    ...
    if (squares.length > 0) {
        ...
        updateScore(pts);
        squares.forEach(sq => sq.forEach(g => toRemove.push(g)));
    }
...
function saveToStorage() {
    ...
    try {
        localStorage.setItem('gemMatchSave', JSON.stringify({
            score: state.score,
            moves: state.moves,
            level: state.level,
            goal: state.goal,
            grid: state.grid.map(row => row.map(g => ({ type: g.type, power: g.power })))
        }));
    } catch (e) { }
}
...
function loadFromStorage() {
    try {
        const raw = localStorage.getItem('gemMatchSave');
        if (!raw) return false;
        ...
        state.grid = data.grid.map(row => row.map(g => ({
            type: g.type, power: g.power,
...
document.getElementById('disclaimer-ok-btn').addEventListener('click', () => {
    document.getElementById('disclaimer-ok-btn').blur();
    document.getElementById('disclaimer-box').classList.remove('show');
});
...
loadBtn.addEventListener('click', () => {
    ...
    if (code.toLowerCase() === 'konami') {
        ...
        for (let r = 0; r < GRID_ROWS; r++) {
            for (let c = 0; c < GRID_COLS; c++) {
                if (state.grid[r][c].type !== GEM_ROCK && state.grid[r][c].power === POWER_NONE) {
                    targets.push({ r, c });
                }
            }
        }
        if (targets.length >= 2) {
            ...
            state.grid[targets[0].r][targets[0].c].power = POWER_BOMB;
            state.grid[targets[1].r][targets[1].c].power = POWER_WILD;
        }
        loadInput.value = "";
        showMessage("↑ ↑ ↓ ↓ ← → ← → B A");
        return;
    ...
    if (code === 'America' || code === 'merica' || code === 'murica') {
        COLORS = ['#ef4444', '#ffffff', '#000080', '#dc2626', '#ffffff', '#3b82f6', '#475569'];
        state.eggMode = 'america';
        loadInput.value = "";
        showMessage("'MURICA!");
        return;
    }
